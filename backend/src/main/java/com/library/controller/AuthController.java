...
public class AuthController {
 ...
 public record Register(@NotBlank String name,@Email @NotBlank String email,@NotBlank @Size(min=10) String password,@NotBlank String phone,@NotBlank String membershipId,boolean librarianApplication){}
 public record Login(@Email @NotBlank String email,@NotBlank String password){}
 @PostMapping("/register") public Map<String,Object> register(@Valid @RequestBody Register r){if(users.existsByEmailIgnoreCase(r.email()))throw new ApiException(HttpStatus.CONFLICT,"Email is already registered");if(users.existsByMembershipId(r.membershipId()))throw new ApiException(HttpStatus.CONFLICT,"Membership ID is already registered");var u=new User();u.setName(r.name());u.setEmail(r.email().toLowerCase());u.setPassword(encoder.encode(r.password()));u.setPhone(r.phone());u.setMembershipId(r.membershipId());u.setRole(r.librarianApplication()?User.Role.LIBRARIAN:User.Role.MEMBER);u.setStatus(r.librarianApplication()?User.UserStatus.PENDING:User.UserStatus.ACTIVE);users.save(u);var p=new NotificationPreference();p.setUser(u);prefs.save(p);if(!r.librarianApplication())mail.send(u.getEmail(),"Library account created","Welcome to the library, "+u.getName()+".");return Map.of("success",true,"status",u.getStatus().name(),"message",r.librarianApplication()?"Librarian application submitted for administrator approval":"Account created","user",safe(u));}
