...
const roleHome = role => role==='ADMIN'?'/admin/dashboard':role==='LIBRARIAN'?'/librarian/dashboard':'/member/dashboard'
function Guard({ children, role }) { const { user, ready } = useAuth(); if (!ready) return <div className="loading-screen"><span className="spinner"/> Loading your library…</div>; if (!user) return <Navigate to="/login" replace/>; if (role && user.role !== role) return <Navigate to={roleHome(user.role)} replace/>; return children }
...
export default function App(){
 ...
 return <Routes>
  ...
  <Route path="/login" element={!ready?<div className="loading-screen"><span className="spinner"/></div>:user?<Navigate to={roleHome(user.role)}/>:<Login/>}/>
  ...
  <Route path="/admin" element={<Workspace role="ADMIN"/>}><Route path="dashboard" element={<AdminPanel/>}/></Route>
